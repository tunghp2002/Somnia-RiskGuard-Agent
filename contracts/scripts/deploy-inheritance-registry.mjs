...
async function main() {
  ...
  const artifactPath = resolve(
    ...
    "RiskGuardInheritanceRegistry.json"
  );
  const artifact = JSON.parse(readFileSync(artifactPath, "utf8"));
  const factory = new ContractFactory(artifact.abi, artifact.bytecode.object, wallet);

  console.log(`Deploying RiskGuardInheritanceRegistry with ${wallet.address}`);
  const contract = await factory.deploy();
  const deploymentTx = contract.deploymentTransaction();
  console.log(`RiskGuardInheritanceRegistry deploy tx: ${deploymentTx?.hash ?? "pending"}`);
  await contract.waitForDeployment();
  console.log(`RiskGuardInheritanceRegistry deployed: ${await contract.getAddress()}`);
}
